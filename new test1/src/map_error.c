...
static void read_map(t_game *game, char *map_file, int config_lines)
{
    ...
    while ((line = get_next_line(fd)))
    {
        ...
        if (is_map_line(line))
        {
            if (i == 6 && ft_strlen(line) > 25)
                ft_printf("Raw Line[25]: char=%c, ASCII=%d\n", line[25], (int)line[25]);
            ...
            temp_map[i] = clean_line;
            if (len > max_cols)
                max_cols = len;
            if (i == 6 && len > 25)
                ft_printf("Temp Map[6][25]: char=%c, ASCII=%d\n", temp_map[i][25], (int)temp_map[i][25]);
            i++;
        }
    ...
    while (i < game->rows)
    {
        ...
        if (i == 9)
            ft_printf("Map row 9, col 12: char=%c, ASCII=%d\n", game->map[9][12], (int)game->map[9][12]);
        if (i == 13)
            ft_printf("Map row 13, col 8: char=%c, ASCII=%d\n", game->map[13][8], (int)game->map[13][8]);
        if (i == 6)
            ft_printf("Map row 6, col 25: char=%c, ASCII=%d\n", game->map[6][25], (int)game->map[6][25]);
        free(temp_map[i]);
        i++;
    }
    game->map[i] = NULL;
    free(temp_map);
    game->columns = max_cols;
    ft_printf("Config count: %d, Max columns: %d, game->columns: %d\n", config_count, max_cols, game->columns);
    if (config_count != 6 || game->floor_color == -1 || game->ceiling_color == -1)
        error_msg(game, INVALID_FORMAT);
}

static int check_cub_extension(const char *map_file)
{
