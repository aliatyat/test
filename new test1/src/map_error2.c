...
static void set_player(t_game *game, int x, int y, char dir)
{
    ...
    game->map[y][x] = '0';
    ft_printf("Player set at (%d, %d), dir: %c, pos: (%.2f, %.2f), angle: %.2f\n",
              x, y, dir, game->player.x, game->player.y, game->player.angle);
}
...
static void check_player(t_game *game)
{
    int player_count = 0;
    int y = 0;
    ft_printf("Starting player check: rows=%d, cols=%d\n", game->rows, game->columns);
    while (y < game->rows)
    {
        int x = 0;
        while (x < game->columns)
        {
            char c = game->map[y][x];
            if (y == 9 && x == 12)
                ft_printf("At (12, 9): char=%c, ASCII=%d\n", c, (int)c);
            if (ft_strchr("NSEW", c))
            {
                ft_printf("Found player at (%d, %d): %c\n", x, y, c);
                set_player(game, x, y, c);
                player_count++;
            }
            else if (!ft_strchr("01 ", c))
            {
                ft_printf("Invalid entity at (%d, %d): %c, (%d)\n", x, y, c, (int)c);
...
static char **copy_map(char **map, int rows, int cols)
{
    ...
    while (i < rows)
    {
        ...
        copy[i][cols] = '\0';
        if (i == 6)
            ft_printf("Copy Map[6][25]: char=%c, ASCII=%d\n", copy[i][25], (int)copy[i][25]);
        i++;
    }
    copy[i] = NULL;
    if (DEBUG)
...
static void check_enclosure(t_game *game)
{
    ...
    ft_printf("Flood fill starting at (%d, %d)\n", start_x, start_y);
    ...
    while (y < game->rows)
    {
        ...
        {
            if (y == 6 && x == 25)
                ft_printf("Checking (25, 6): char=%c, original=%c\n", map_copy[y][x], game->map[y][x]);
            if (map_copy[y][x] == '0')
            {
