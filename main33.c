...
/*   Updated: 2025/05/13 19:21:51 by zjamaien         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	check_args_and_init(t_shell *shell, int argc, char **argv,
		char **envp)
{
	...
	int		atoi;
	char	*itoa;
	if (usage_minishell(argv[1], argc))
		return (1);
	if (!init_shell(shell, envp))
		return (EXIT_FAILURE);
	env_val = get_env_value(shell->env, "SHLVL");
	atoi = ft_atoi(env_val) + 1;
	itoa = ft_itoa(atoi);
	update_env_var(shell->env, "SHLVL", itoa);
	free(itoa);
	free(env_val);
	return (0);
}
...
static void	run_shell_loop(t_shell *shell)
{
	...
	while (1)
	{
		...
		int number = ft_atoi(get_env_value(shell->env, "SHLVL"));
		if (number >= 999)
...
int	main(int argc, char **argv, char **envp)
{
	...
	if (check_args_and_init(&shell, argc, argv, envp))
		return (1);
...
// 	if (!init_shell(&shell, envp))
// 		return (EXIT_FAILURE);
// 	update_env_var(shell.env, "SHLVL", ft_itoa(ft_atoi(get_env_value(shell.env,
// 						"SHLVL")) + 1));
// 	while (1)
// 	{
// 		handle_signal();
...
// 		process_input(&shell, input);
// 		save_exit = shell.exit_status;
// 		if (ft_atoi(get_env_value(shell.env, "SHLVL")) >= 999)
// 			update_env_var(shell.env, "SHLVL", "1");
// 		close_all_pipes(shell.commands);
// 		free(input);
// 	}
