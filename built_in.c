...
/*   Updated: 2025/05/12 15:47:03 by zjamaien         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	process_export_entry(char *arg, t_shell *shell);
int	is_invalid_identifier(char *arg, char *equal_input);
int	handle_assignment(char *arg, char *equal_sign, t_shell *shell);
int	handle_empty_assignment(char *arg, t_shell *shell);
...
int	process_export_entry(char *arg, t_shell *shell)
{
	...
	if (is_invalid_identifier(arg, equal_input))
		return (print_invalid_identifier(arg));
	if (equal_sign)
		return (handle_assignment(arg, equal_sign, shell));
	if (!equal_input)
		return (handle_empty_assignment(arg, shell));
	return (0);
}
...
int	ft_export(t_command *cmd, t_shell *shell)
{
	...
	status = 0;
	if (!cmd->args[1])
		return (print_sorted_env(shell->env, cmd->out_fd), 0);
	while (cmd->args[i])
	{
		if (process_export_entry(cmd->args[i], shell))
			status = 1;
		i++;
	}
	return (status);
...
// 				status = 1;
// 		}
// 		if (ft_isdigit(cmd->args[i][0]) || equal_new && *(equal_new - 1) == ' ')
// 		{
// 			ft_printf("minishell: export: `%s': not a valid identifier\n",
// 				cmd->args[i]);
// 			status = 1;
