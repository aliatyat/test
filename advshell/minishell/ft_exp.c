...
/*   Updated: 2025/05/05 18:11:42 by alalauty         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	while (*ptr && buf_index < sizeof(buffer) - 1)
	{
				...
				2) != 0)
		{
			...
			if (*ptr == '?')
			{
				...
				{
					exit_status1 = ft_itoa(g_signal_status);
				}
				else
				{
			...
			{
				ft_strlcpy(buffer + buf_index, shell->argv_z, sizeof(buffer)
					- buf_index);
					buf_index += ft_strlen(shell->argv_z);
